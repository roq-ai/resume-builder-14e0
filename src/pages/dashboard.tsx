...
import { getEducations } from 'apiSdk/educations';
import { EducationInterface } from 'interfaces/education';

import { TemplateListPage as ShowCaseList } from 'pages/templates';

function DashboardPage() {
  const { session } = useSession();

  const experienceStat = useSWR<PaginatedInterface<ExperienceInterface>>(
    () => `/experiences`,
    () => getExperiences({ limit: 1, offset: 0 }),
  );

  const resumeStat = useSWR<PaginatedInterface<ResumeInterface>>(
    () => `/resumes`,
    () => getResumes({ limit: 1, offset: 0 }),
  );

  const educationStat = useSWR<PaginatedInterface<EducationInterface>>(
    () => `/educations`,
    () => getEducations({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Experiences',
      result: experienceStat?.data?.totalCount,
      isLoading: experienceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Resumes',
      result: resumeStat?.data?.totalCount,
      isLoading: resumeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Educations',
      result: educationStat?.data?.totalCount,
      isLoading: educationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
