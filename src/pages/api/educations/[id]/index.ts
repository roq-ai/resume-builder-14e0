...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const allowed = await prisma.education
    .withAuthorization({
      roqUserId,
  ...
  async function getEducationById() {
    const data = await prisma.education.findFirst(convertQueryToPrismaUtil(req.query, 'education'));
    return res.status(200).json(data);
  }

  async function updateEducationById() {
    await educationValidationSchema.validate(req.body);
  ...
  async function deleteEducationById() {
